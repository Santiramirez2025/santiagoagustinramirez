...
let _qReady;
let _lReady = null;
// Leads: gente que calculó su costo del desorden y dejó contacto, haya comprado o no.
function ensureLeadsTable() {
  ...
  if (!_lReady) {
    _lReady = sql`
      ...
      .then(function () { return sql`CREATE INDEX IF NOT EXISTS leads_email_idx ON leads (email)`; })
      .then(function () { return sql`CREATE INDEX IF NOT EXISTS leads_created_idx ON leads (created_at DESC)`; })
      .then(function () { return true; })
      .catch(function (e) { console.error('[db] ensureLeadsTable', e.message); _lReady = null; return false; });
  }
  return _lReady;
}
...
async function saveLead(l) {
  await sql`
    INSERT INTO leads (id, email, phone, name, inputs, resultado, utm)
    VALUES (${l.id}, ${l.email || null}, ${l.phone || null}, ${l.name || null},
            ${JSON.stringify(l.inputs || {})}, ${JSON.stringify(l.resultado || {})}, ${JSON.stringify(l.utm || {})})
...
async function listLeads(limit) {
  ...
  return await sql`
    SELECT l.*, o.id AS compro
      FROM leads l
      LEFT JOIN orders o ON o.buyer_email = l.email AND o.status = 'approved'
...
// Pedidos recientes para el centro de operaciones, con el turno reservado y si
// el cliente ya completó el cuestionario.
async function listOrders(limit) {
  ...
  return sql`
    SELECT o.*,
           b.slot_start,
           b.slot_end,
           (q.id IS NOT NULL) AS has_questionnaire,
           q.updated_at AS questionnaire_at
    FROM orders o
    LEFT JOIN bookings b ON b.order_id = o.id
    LEFT JOIN questionnaires q ON q.order_id = o.id
...
module.exports = { sql, ensureLeadsTable, saveLead, listLeads, ensureEventsTable, insertEvent, createOrder, getOrder, markPaidOnce, createBookingHold, confirmBooking, takenSlots, ensureQuestionnaireTable, saveQuestionnaire, getQuestionnaire, listOrders };
